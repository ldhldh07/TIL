...
patient에 별개의 필드가 새로 생긴게 아님

hospital_patient_doctors가 생겼는데 아까 Reservation이랑 똑같이 생김

- 알아서 중개 테이블을 만들어준것

patient에 작성하던 doctor에 작성하던 상관 없다
...
- patient에 쓰면 patient가 doctor 참조 doctor가 patient 역참조



예약을 잡는 주체는 환자와 의사

Reservation 만들었을때는 Reservation이 객체 선언해서 어색했음

하지만 ManytoManyField는 doctor와 patient가 주체가 되고 이게 자연스러움
...
target model이 source model을 참조할 때 사용할

ForeignKey()의 relate_name과 동일
...
ManyToManyField랑 중개 테이블 둘 다 만들어줘야 함
...
- 여전히 Reservation을 호출해서 생성하는 것 또한 가능
...
add()에 추가 데이터 넣으려면 through_defaults에 딕셔너리 넣음
...
- 우리가 이전에 모델 생성 시 objects 라는 매니저를 통해 queryset api를 사용했던 것처럼related manager를 통해 queryset api를 사용할 수 있게 됨
...
user.article_set이 두개가 충돌하게 됨

related_name을 필수적으로 사용해야 하는 상화

- 일반적으로 M:N을 바꿈
- M:N에서 related_name을 like_articles로 만듬
...
variable routing으로 article_pk보내줌
...
하나는 settings.AUTH_USER_MODEL인데 이건 models.py만
...
바로 get_user_model().objects.get할수도 있지만
...
article.user만 해도 article.user.username처럼 작동함
...
그래서 필드명은 followings, related_name이 followers

자기자신 참조는 문자열로 'self'

symmetrical은 True로 하면 무조건 맞팔되버려서 False
...
누구인지 알아야 하니까 user_pk를 variable rounting
