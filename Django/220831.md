...
`models.py`를 가지고 `makemigrations`해서 `0001_initial`만들었는데 이게 실제 설계도(blueprint)
...
설계도까지 만든건 DB에 영향 X `migrate`까지 해야지 적용
...
1번은 기존의 데이터가 있다면 키보드에 입력한 값 입력하겠다 컬럼의 기본값 직접 입력

2번은 여기서 나가서 코드에 디폴트 값 적어라 모델 필드에 직접 입력

```bash
Please enter the default value now, as valid Python # 기본값 입력해라 파이썬에 유효하게
You can accept the default 'timezone.now' by pressing 'Enter' or you can provide another value. # 그냥 엔터 누르면 timezone.now 자동으로 입력
...
SQL만 알아 들을 수 있는 DB가 어떻게 이 설계도를 이해하고 동기화를 이룰 수 있을까?
...
한마디로  SQL을 사용하지 않고 데이터베이스를 조작할 수 있게 만들어놓은 매개체
...
```
 ...
 save를 해야 입력됨 한줄 쓰고 이거 전까지는 그냥 인스턴스
...
setting에서 보여지는 시간 한국시간으로 해도 저장되는건 UTC
...
인스턴스 변수 이름은 아무거나 해도됨 article아니어도 됨
...
1. return new querysets : 쿼리셋을 받느냐
2. do not return querysets : 쿼리셋을 받지 않느냐
...
없다고 오류 뜨는게 아니라 빈 쿼리셋 줌

하나면 하나 넣어서 줌

pk로 조회할때 적합한 방법이 아님 get으로 조회
...
미리 끝내두심 ulr 분리랑 템플렛-base.html이랑 아티클템플렛이랑
...
from .models import Article # 이 폴더에 있는 모델스에서 아티클을 쓸수 있게
...
CREATE 로직을 구현하기 위해서는 몇개의 view 함수가 필요할까?
...
def new(request):
    return render(request, 'articles/new.html') # 이제 작성할 것이니 미ㅁ
```
...
링크 넣어주고 new에 input에 값 입력하면 DB에 추가됨
