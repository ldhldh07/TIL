...
setting.py에 MEDIA_ROOT로 실제 DIR경로 추가
...
폼을 불러올 때 request.POST뿐 아니라 request.FILES도 불러옴
...
ImageField는 업로드 드렉토리와 파일 이름을 설정하는 2가지 방법을 제공
...
저장 안한 거에 ImageSpecField 추가 import
...
imagespecfield에는 아까랑 비슷한데 소스만 추가
...
파일은 media/CACHE라는 폴더가 생겨서 그 안에 난수로 저장되어있음

생성 시기- 출력이 이루어졌을때

- 그래서 다른 파일들 있던 게시글 들어가서 출력되면 그 게시글에 출력하는 파일도 썸네일 생성됨

개발자도구 Network를 보면 size가 없고 메모리캐시라고 되어있음 장고가 아니라 크롬이 이 이미지를 memory cache에 저장해두었다가
